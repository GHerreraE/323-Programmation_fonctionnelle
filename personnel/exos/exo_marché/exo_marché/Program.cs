...
namespace exo_marché
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var vendeursPeches = products.Where(v => v.ProductName == "Pêches").Count();
            Console.WriteLine("Il y a " + vendeursPeches + " vendeurs de pêches");

            // Quel producteur a le plus de pastèques à vendre ? Où est-il ? Combien en a-t-il ?
            var plusPasteques = products.Where(p => && (p.ProductName == "Pastèques") && (p.Quantity == int.MaxValue));
            Console.WriteLine(plusPasteques);
